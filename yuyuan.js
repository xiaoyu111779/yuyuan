...
【只总结剧情事实,不要总结或评价角色的性格、语气、说话风格——那些以角色卡为准,不归你管】。纯文本输出,不要解释。`;
  ...
  async function forceSyncToMain(text) {
    ...
    const _d = loadData();
    if (/^\s*\[微信\]/.test(_t)) { if (_d.wxDmSync === false) return; }
    else if (/^\s*\[小红书·私信\]/.test(_t)) { if (_d.xhsDmSync === false) return; }
    else if (!_d.syncToMain) return;
    pushXhsDirective();
    try {
      await createChatMessages([{
...
JSON 里额外给:"clues":[发现的具体重合线索,没有就空数组],"evidence":0~40(这条【新增】实锤程度:泛泛/巧合 0~5;一个具体重合 10~20;多个强重合 25~40)。【线索必须真实,严禁编造/硬掰同音】拿不准就留空、evidence=0。\n`;
  ...
  function renderBody(d) {
    ...
    if (d.currentApp === 'yangchar') return xichangOn() ? renderYangchar(d) : renderHome(d);
...
严格只输出 JSON 数组(尽量 ${n} 个;设定里能出的题不够就少给几个,别硬编):["问题1","问题2"]`;
  ...
  function devKnock() {
    ...
    if (_devKnockN >= 7) {
      ...
      toastr.success(on ? '🔧 开发者模式已关闭,小剧场 / 养 ta 已隐藏' : '🔧 开发者模式已开启,小剧场 / 养 ta 已解锁~');
      refreshXhs();
    }
  }
  ...
  // ====== 养 ta · 形象馆(AI 生图)======
  const YC_CATS = [
    {key:'base',lab:'基础',multi:false,opts:[['男孩','1boy, solo'],['女孩','1girl, solo']]},
    {key:'style',lab:'画风',multi:false,opts:[['Q版萌','chibi'],['普通比例','']]},
    {key:'frame',lab:'构图',multi:false,opts:[['半身','upper body'],['膝盖以上','cowboy shot'],['全身','full body']]},
    {key:'hair',lab:'发型',multi:false,opts:[['乱短发','messy short hair'],['短发','short hair'],['中长发','medium hair'],['长发','long hair'],['卷发','wavy hair'],['双马尾','twintails'],['丸子头','hair bun']]},
    {key:'hairdetail',lab:'发型细节',multi:true,opts:[['眉间碎发','hair between eyes'],['呆毛','ahoge'],['遮一只眼','hair over one eye']]},
    {key:'haircolor',lab:'发色',multi:false,opts:[['黑','black hair'],['棕','brown hair'],['白银','grey hair'],['白','white hair'],['金','blonde hair'],['粉','pink hair'],['蓝','blue hair'],['紫','purple hair'],['红','red hair']]},
    {key:'eyes',lab:'瞳色',multi:false,opts:[['灰','grey eyes'],['蓝','blue eyes'],['红','red eyes'],['金','yellow eyes'],['绿','green eyes'],['紫','purple eyes'],['粉','pink eyes']]},
    {key:'expr',lab:'表情',multi:false,opts:[['开心笑','happy, smile'],['平静','expressionless'],['害羞','blush, embarrassed'],['哭','crying, tears'],['坏笑','smirk'],['嘟嘴','pout']]},
    {key:'aura',lab:'氛围',multi:true,opts:[['可爱','kawaii'],['开心光环','happy aura'],['闪亮','sparkles'],['柔光','soft lighting']]},
    {key:'pose',lab:'姿势',multi:false,opts:[['看镜头','looking at viewer'],['跑向镜头','running'],['比耶','peace sign'],['招手','waving'],['坐着','sitting'],['叉腰','hands on hips']]},
    {key:'feat',lab:'兽耳/特征',multi:true,opts:[['猫耳','cat ears, animal ears'],['兔耳','rabbit ears, animal ears'],['狐耳','fox ears, animal ears'],['龙角','horns'],['虎牙','fang'],['泪痣','mole under eye']]},
    {key:'cloth',lab:'服装',multi:false,opts:[['黑背心','black tank top'],['卫衣','hoodie'],['白衬衫','white shirt'],['校服','school uniform'],['女仆装','maid'],['毛衣','sweater']]},
    {key:'bg',lab:'背景',multi:false,opts:[['白底','white background'],['简单','simple background'],['樱花','cherry blossoms'],['星空','starry background']]},
  ];
  const YC_QUALITY = 'solo focus, no text, masterpiece, best quality';
  const YC_DEFAULT = { base:'1boy, solo', style:'chibi', haircolor:'black hair', eyes:'grey eyes', hairdetail:['hair between eyes'], expr:'happy, smile', aura:['kawaii','happy aura'], feat:['cat ears, animal ears'], cloth:'black tank top', bg:'white background' };

  const YC_DAILY_CAP = 10, YC_GAIN = 3;
  const YC_LEVELS = [{p:0,name:'初识'},{p:30,name:'眼熟'},{p:80,name:'朋友'},{p:180,name:'暧昧'},{p:350,name:'心动'},{p:600,name:'恋人'},{p:1000,name:'挚爱'}];
  const YC_TOUCHES = [
    {key:'head',label:'摸摸头',emoji:'🫳'},{key:'ear',label:'戳兽耳',emoji:'🐾'},{key:'cheek',label:'戳脸',emoji:'👉'},
    {key:'hand',label:'牵手',emoji:'🤝'},{key:'hair',label:'揉头发',emoji:'💆'},{key:'hug',label:'抱抱',emoji:'🫂'},
    {key:'waist',label:'搂腰',emoji:'🫰'},{key:'kiss',label:'亲亲',emoji:'💋'},{key:'pinch',label:'捏脸',emoji:'🤏'},
  ];
  function ycToday() { const n = new Date(); return n.getFullYear() + '-' + (n.getMonth() + 1) + '-' + n.getDate(); }
  function ycLevel(points) {
    let idx = 0; for (let i = 0; i < YC_LEVELS.length; i++) { if (points >= YC_LEVELS[i].p) idx = i; }
    const cur = YC_LEVELS[idx], next = YC_LEVELS[idx + 1];
    const pct = next ? Math.max(0, Math.min(100, Math.round((points - cur.p) / (next.p - cur.p) * 100))) : 100;
    return { name: cur.name, lv: idx + 1, pct, next: next ? next.p : null };
  }
  let ycReplyBusy = false;
  function ycCleanReply(s) {
    let raw = String(s || '');
    // 剥掉开头的消息信封 [xxx|xxx]:
    let t = raw.replace(/^\s*\[[^\]\n]{0,60}[|｜][^\]\n]{0,60}\]\s*[:：]?\s*/, '');
    // ta 真正的话在状态栏之前 —— 从第一个状态栏标记处截断
    let cut = t.length;
    [t.search(/<\s*[A-Za-z\/][A-Za-z0-9_]*/), t.search(/\[[A-Za-z\u4e00-\u9fa5]{1,16}\s*[|｜]/), t.search(/```/)].forEach(function (m) { if (m >= 0 && m < cut) cut = m; });
    function scrub(x) {
      x = x.replace(/<\/?[A-Za-z][^>]*>/g, ' ').replace(/\[[^\]\n]*[|｜][^\]\n]*\]/g, ' ');
      x = x.replace(/[ \t]*\n[ \t]*/g, ' ').replace(/\s{2,}/g, ' ').trim();
      x = x.replace(/^["「'\u201c]+|["」'\u201d]+$/g, '').trim();
      return x;
    }
    let head = scrub(t.slice(0, cut));
    if (!head) head = scrub(t);
    return head.slice(0, 220);
  }
  async function genCharTouch(partLabel, levelName) {
    const role = getRoleDesc() || ''; const wb = await getWorldbookContent(); const cname = getCharName();
    const _cp = (loadData().charPortrait) || {}; const look = (_cp.look || _cp.prompt || '').trim();
    const sys = `你在「养 ta」养成互动里扮演 ${cname}。{{user}} 刚刚【${partLabel}】ta。
【${cname} 的人设/性格】:
${role}
${wb ? `【世界书/设定】:\n${wb}\n` : ''}${look ? `【ta 现在这张形象的样子(很重要,反应要贴合)】:${look}\n(发型、衣着、正拿着/正在做的事,都可自然带入反应。)\n` : ''}(你俩当前关系/好感见下。当前好感阶段:${levelName},越高 ta 越亲近黏人。)
请用 ${cname} 的口吻、按【人设 + 你俩当前关系阶段】对这个动作给【一到两句反应】:关系/好感没到、或动作偏亲密(亲亲/搂腰等),可以害羞/躲/嘴硬/拒绝;到了就自然接受甚至回应,可往亲昵暧昧走(按关系阶段把握分寸,可含成人向但别 OOC)。【硬性格式】只输出 ta 说的这一句话本身。绝对不要状态栏,不要 <xxx> 这类尖括号标签,不要 [字段|…] / [字段:…] 这类方括号字段,不要旁白/动作描写/解释。`;
    const raw = await callXhsAPI(sys, `{{user}}${partLabel}了 ta。ta 只回一句话:`, { noContext: true });
    return raw ? String(raw).trim().replace(/^["「'\u201c]+|["」'\u201d]+$/g, '').slice(0, 300) : '';
  }
  async function ycTouch(partKey) {
    const t = YC_TOUCHES.find(x => x.key === partKey); if (!t) return;
    let d = loadData(); d.charAffection = d.charAffection || { points: 0, day: '', count: 0, lastReply: '' };
    const today = ycToday();
    if (d.charAffection.day !== today) { d.charAffection.day = today; d.charAffection.count = 0; }
    const canGain = (d.charAffection.count || 0) < YC_DAILY_CAP;
    await saveData(d);
    const lv = ycLevel(d.charAffection.points || 0);
    toastr.info('ta 正在反应…');
    const reply = await genCharTouch(t.label, lv.name);
    if (!reply) { toastr.error('没出反应,再点一次~'); return; }
    const d2 = loadData(); d2.charAffection = d2.charAffection || { points: 0, day: today, count: 0 };
    if (d2.charAffection.day !== today) { d2.charAffection.day = today; d2.charAffection.count = 0; }
    if (canGain) { d2.charAffection.points = (d2.charAffection.points || 0) + YC_GAIN; d2.charAffection.count = (d2.charAffection.count || 0) + 1; }
    d2.charAffection.lastReply = reply; d2.charAffection.lastPart = t.label;
    await saveData(d2); refreshXhs();
    if (canGain) toastr.success('好感 +' + YC_GAIN); else toastr.info('今天摸够啦,好感不涨了~');
  }

  const YC_PARTS = [
    { key: 'head', label: '头', action: '摸摸头' },
    { key: 'ear', label: '耳朵/兽耳', action: '戳了戳耳朵' },
    { key: 'face', label: '脸', action: '戳了戳脸' },
    { key: 'mouth', label: '嘴/下巴', action: '摸了摸下巴' },
    { key: 'neck', label: '脖子', action: '蹭了蹭脖子' },
    { key: 'hand', label: '手', action: '牵了牵手' },
    { key: 'chest', label: '胸口', action: '贴了贴胸膛' },
    { key: 'waist', label: '腰/肚子', action: '搂了搂腰' },
    { key: 'hip', label: '胯/大腿', action: '摸了摸大腿' },
    { key: 'crotch', label: '裆(没有就跳过)', action: '碰了碰 ta 的裆部' },
  ];
  function ycImgNorm(e, stage) {
    let nx = 0.5, ny = 0.5;
    try {
      const img = stage.querySelector('img');
      const ib = (img || stage).getBoundingClientRect();
      let iy = ((e && e.clientY != null) ? e.clientY : ib.top + ib.height / 2) - ib.top;
      let ix = ((e && e.clientX != null) ? e.clientX : ib.left + ib.width / 2) - ib.left;
      if (img && img.naturalWidth && img.naturalHeight) {
        const sc = Math.min(ib.width / img.naturalWidth, ib.height / img.naturalHeight);
        const dW = img.naturalWidth * sc, dH = img.naturalHeight * sc;
        ny = (iy - (ib.height - dH) / 2) / dH;
        nx = (ix - (ib.width - dW) / 2) / dW;
      } else { ny = iy / ib.height; nx = ix / ib.width; }
    } catch (_) {}
    return { nx: Math.max(0, Math.min(1, nx)), ny: Math.max(0, Math.min(1, ny)) };
  }
  function ycPick(a) { return a[Math.floor(Math.random() * a.length)]; }
  function ycRace(p) { return Promise.race([p, new Promise(function (res) { setTimeout(function () { res(''); }, 15000); })]); }
  function ycFallbackLine(action, food) {
    if (food) return ycPick(['唔…好吃。', '谢了,正好饿了。', '喂我?…也行。', '慢点,又没人跟你抢。', '嗯,这个还不错。', '…投喂可还行。']);
    const a = String(action || '');
    if (/亲|嘴|kiss/.test(a)) return ycPick(['…突然干嘛。', '就一下啊。', '脸都不要了。']);
    if (/抱|搂|腰/.test(a)) return ycPick(['…还挺黏人。', '行吧,就一会儿。']);
    if (/裆|胯|大腿|腿/.test(a)) return ycPick(['手往哪放呢。', '…胆子不小啊。']);
    if (/头|发/.test(a)) return ycPick(['嗯…还挺舒服。', '别给我揉乱了。']);
    if (/手/.test(a)) return ycPick(['…牵着吧。', '手挺凉的。']);
    return ycPick(['嗯?', '…怎么了。', '别戳了。', '在呢。', '想我了?']);
  }
  function ycZone(ny, nx) {
    if ((nx < 0.12 || nx > 0.88) && ny > 0.4 && ny < 0.82) return ycPick(['牵了牵手', '握了握 ta 的手']);
    if (ny < 0.13) return ycPick(['摸摸头', '揉了揉头发']);
    if (ny < 0.24) return (nx < 0.3 || nx > 0.7) ? '戳了戳兽耳' : ycPick(['戳了戳脸', '捏了捏脸颊']);
    if (ny < 0.33) return ycPick(['摸了摸下巴', '蹭了蹭脸']);
    if (ny < 0.42) return ycPick(['摸了摸脖子', '蹭了蹭脖颈']);
    if (ny < 0.50) return ycPick(['摸了摸肩膀', '戳了戳锁骨']);
    if (ny < 0.62) return ycPick(['戳了戳胸口', '贴在 ta 胸膛上']);
    if (ny < 0.73) return ycPick(['搂了搂腰', '戳了戳侧腰', '摸了摸小腹']);
    if (ny < 0.84) return ycPick(['摸了摸胯', '蹭了蹭大腿根']);
    if (ny < 0.92) return '碰了碰 ta 的裆部';
    return ycPick(['摸了摸大腿', '戳了戳腿']);
  }
  const YC_FOODS = [
    { key: 'rice', emoji: '🍙', name: '饭团', fill: 10 },
    { key: 'ramen', emoji: '🍜', name: '拉面', fill: 18 },
    { key: 'cake', emoji: '🍰', name: '蛋糕', fill: 12 },
    { key: 'milktea', emoji: '🧋', name: '奶茶', fill: 9 },
    { key: 'meat', emoji: '🍖', name: '烤肉', fill: 20 },
    { key: 'berry', emoji: '🍓', name: '草莓', fill: 6 },
    { key: 'icecream', emoji: '🍦', name: '冰淇淋', fill: 8 },
    { key: 'chicken', emoji: '🍗', name: '炸鸡', fill: 16 },
  ];
  const YC_FEED_DECAY = 4, YC_FEED_CAP = 8;
  function ycFullnessVal(feed) {
    if (!feed) return 70;
    const hrs = Math.max(0, (Date.now() - (feed.at || Date.now())) / 3600000);
    const base = (feed.fullness != null ? feed.fullness : 70);
    return Math.max(0, Math.min(100, Math.round(base - YC_FEED_DECAY * hrs)));
  }
  async function ycFeed(foodKey) {
    const f = YC_FOODS.find(x => x.key === foodKey); if (!f) return;
    const TOPw = getTop();
    const stage = TOPw.document.querySelector('#xhs-float .yc-stage');
    let d = loadData();
    d.charFeed = d.charFeed || { fullness: 70, at: Date.now(), day: '', count: 0 };
    const today = ycToday();
    if (d.charFeed.day !== today) { d.charFeed.day = today; d.charFeed.count = 0; }
    const cur = ycFullnessVal(d.charFeed);
    d.charFeed.fullness = Math.min(100, cur + f.fill);
    d.charFeed.at = Date.now();
    const feedCanGain = (d.charFeed.count || 0) < YC_FEED_CAP;
    if (feedCanGain) d.charFeed.count = (d.charFeed.count || 0) + 1;
    d.charAffection = d.charAffection || { points: 0, day: today, count: 0 };
    if (feedCanGain) d.charAffection.points = (d.charAffection.points || 0) + 2;
    await saveData(d);
    // 原地更新 饱食度 + 好感条(不刷新,保住动画)
    try {
      const doc = TOPw.document;
      const fv = ycFullnessVal(d.charFeed);
      const ff = doc.getElementById('yc-full-fill'); if (ff) ff.style.width = fv + '%';
      const ft = doc.getElementById('yc-full-txt'); if (ft) ft.textContent = '饱食 ' + fv;
      const lv = ycLevel(d.charAffection.points || 0);
      const bf = doc.getElementById('yc-bar-fill'); if (bf) bf.style.width = lv.pct + '%';
      const bp = doc.getElementById('yc-bar-points'); if (bp) bp.textContent = '好感 ' + (d.charAffection.points || 0) + (lv.next != null ? (' / ' + lv.next) : '');
      const bl = doc.getElementById('yc-bar-lv'); if (bl) bl.textContent = 'Lv.' + lv.lv + ' ' + lv.name;
    } catch (_) {}
    // 食物飞向 ta
    if (stage) {
      try {
        const rect = stage.getBoundingClientRect();
        const el = TOPw.document.createElement('span');
        el.className = 'yc-heart'; el.textContent = f.emoji;
        el.style.left = (rect.width * (0.35 + Math.random() * 0.3)) + 'px';
        el.style.top = (rect.height * 0.6) + 'px'; el.style.fontSize = '30px';
        stage.appendChild(el); setTimeout(function () { try { el.remove(); } catch (_) {} }, 1500);
      } catch (_) {}
    }
    let reply = '';
    if (!ycReplyBusy) {
      ycReplyBusy = true;
      try { reply = await ycRace(genCharTouch('喂 ta 吃了' + f.name, ycLevel((loadData().charAffection || {}).points || 0).name)); } catch (_) {}
      ycReplyBusy = false;
    }
    if (!reply) reply = ycFallbackLine('喂', f);
    try { const d2 = loadData(); d2.charAffection = d2.charAffection || {}; d2.charAffection.lastReply = reply; d2.charAffection.lastPart = '喂' + f.name; await saveData(d2); } catch (_) {}
    if (stage) {
      try {
        const rect = stage.getBoundingClientRect();
        const t = TOPw.document.createElement('div'); t.className = 'yc-floattxt'; t.textContent = reply;
        t.style.left = Math.max(6, rect.width * 0.2) + 'px'; t.style.top = (rect.height * 0.48) + 'px';
        stage.appendChild(t); setTimeout(function () { try { t.remove(); } catch (_) {} }, 5200);
      } catch (_) {}
    }
  }
  async function ycTap(e, $btn) {
    const TOPw = getTop();
    const stage = $btn && $btn[0];
    if (!stage) return;
    const rect = stage.getBoundingClientRect();
    let x = (e && e.clientX != null) ? e.clientX - rect.left : rect.width / 2;
    let y = (e && e.clientY != null) ? e.clientY - rect.top : rect.height / 2;
    ...
    for (let i = 0; i < n; i++) {
      const h = TOPw.document.createElement('span');
      h.className = 'yc-heart';
      h.textContent = HS[Math.floor(Math.random() * HS.length)];
      h.style.left = (x + (Math.random() * 42 - 21)) + 'px';
      h.style.top = y + 'px';
      h.style.fontSize = (17 + Math.random() * 13) + 'px';
      h.style.animationDelay = (i * 0.06) + 's';
      stage.appendChild(h);
      setTimeout(function () { try { h.remove(); } catch (_) {} }, 1500);
    }
    // 区域:有标注→就近匹配你标的点(准);没标注→通用摸摸(B 兜底)
    const _nm = ycImgNorm(e, stage); const nx = _nm.nx, ny = _nm.ny;
    const _hs = ((loadData().charPortrait) || {}).hotspots || [];
    let zone;
    if (_hs.length) {
      let best = null, bd = 1e9;
      for (const h of _hs) { const dd = (h.x - nx) * (h.x - nx) + (h.y - ny) * (h.y - ny); if (dd < bd) { bd = dd; best = h; } }
      zone = (best && best.action) || '戳了戳 ta';
    } else {
      zone = ycPick(['戳了戳 ta', '摸了摸 ta', '蹭了蹭 ta', '挠了挠 ta', '轻轻碰了碰 ta']);
    }
    // 好感立即结算
    let d = loadData();
    d.charAffection = d.charAffection || { points: 0, day: '', count: 0, lastReply: '' };
    const today = ycToday();
    if (d.charAffection.day !== today) { d.charAffection.day = today; d.charAffection.count = 0; }
    const canGain = (d.charAffection.count || 0) < YC_DAILY_CAP;
    if (canGain) { d.charAffection.points = (d.charAffection.points || 0) + YC_GAIN; d.charAffection.count = (d.charAffection.count || 0) + 1; }
    await saveData(d);
    // 原地更新好感条(不刷新,保住动画)
    try {
      const doc = TOPw.document;
      const lv = ycLevel(d.charAffection.points || 0);
      const left = Math.max(0, YC_DAILY_CAP - (d.charAffection.count || 0));
      const f = doc.getElementById('yc-bar-fill'); if (f) f.style.width = lv.pct + '%';
      const p = doc.getElementById('yc-bar-points'); if (p) p.textContent = '好感 ' + (d.charAffection.points || 0) + (lv.next != null ? (' / ' + lv.next) : '');
      const l2 = doc.getElementById('yc-bar-lv'); if (l2) l2.textContent = 'Lv.' + lv.lv + ' ' + lv.name;
      const lf = doc.getElementById('yc-bar-left'); if (lf) lf.textContent = '今天还能摸 ' + left + ' 下' + (left === 0 ? ' (摸够啦,明天再来~)' : '');
    } catch (_) {}
    // ta 回复 → 飘字(节流:一次只跑一个回复,点快了只冒爱心、不堆请求)
    if (ycReplyBusy) return;
    ycReplyBusy = true;
    let think = null;
    try {
      think = TOPw.document.createElement('div');
      think.className = 'yc-floattxt yc-thinking';
      think.textContent = '♡…';
      let txk = x - 22; txk = Math.max(6, Math.min(rect.width * 0.6, txk));
      think.style.left = txk + 'px';
      think.style.top = Math.max(8, y - 22) + 'px';
      stage.appendChild(think);
    } catch (_) {}
    const lvNow = ycLevel(d.charAffection.points || 0);
    let reply = '';
    try { reply = await ycRace(genCharTouch(zone, lvNow.name)); } catch (_) {}
    ycReplyBusy = false;
    try { if (think) think.remove(); } catch (_) {}
    if (!reply) reply = ycFallbackLine(zone);
    try { const d2 = loadData(); d2.charAffection = d2.charAffection || {}; d2.charAffection.lastReply = reply; d2.charAffection.lastPart = zone; await saveData(d2); } catch (_) {}
    try {
      if (!(stage && stage.isConnected)) return;
      const t = TOPw.document.createElement('div');
      t.className = 'yc-floattxt';
      t.textContent = reply;
      let tx = x - 70; tx = Math.max(6, Math.min(rect.width * 0.42, tx));
      t.style.left = tx + 'px';
      t.style.top = Math.max(8, y - 26) + 'px';
      stage.appendChild(t);
      setTimeout(function () { try { t.remove(); } catch (_) {} }, 5200);
    } catch (_) {}
  }

  function buildYcPrompt(d) {
    const b = d.ycBuild || {};
    let parts = [];
    YC_CATS.forEach(c => { const v = b[c.key]; if (c.multi) { (v || []).forEach(x => x && parts.push(x)); } else if (v) parts.push(v); });
    if ((b.custom || '').trim()) parts.push(b.custom.trim());
    let p = parts.join(', ');
    if (!/\bsolo\b/i.test(p)) p = 'solo, ' + p;          // 强制单人
    if (!/\b1(boy|girl)\b/i.test(p)) p = '1boy, ' + p;   // 没选基础也兜个单人
    return p + ', ' + YC_QUALITY;
  }

  async function genCharImage(prompt) {
    try {
      const ctx = getCtx();
      if (!ctx || !ctx.executeSlashCommandsWithOptions) { toastr.error('拿不到酒馆斜杠命令接口,请确认酒馆助手/图像生成已启用'); return ''; }
      const safe = String(prompt).replace(/[|\n\r]/g, ' ').replace(/"/g, '').trim();
      const res = await ctx.executeSlashCommandsWithOptions('/sd quiet=true ' + safe);
      let url = res && (res.pipe != null ? res.pipe : (typeof res === 'string' ? res : ''));
      return String(url || '').trim();
    } catch (e) { toastr.error('生成出错:' + (e && e.message || e)); return ''; }
  }

  async function openYangchar() {
    const d = loadData();
    d.currentApp = 'yangchar'; d.currentRoute = 'yc-home'; d.routeContext = {};
    if (!d.ycBuild) d.ycBuild = JSON.parse(JSON.stringify(YC_DEFAULT));
    await saveData(d); refreshXhs();
  }
  async function ycToBuild() { const d = loadData(); d.currentRoute = 'yc-build'; await saveData(d); refreshXhs(); }
  async function ycHome() { const d = loadData(); d.currentRoute = 'yc-home'; await saveData(d); refreshXhs(); }
  async function ycSetTag(cat, val, multi) {
    const d = loadData(); d.ycBuild = d.ycBuild || {};
    if (String(multi) === '1') { const a = d.ycBuild[cat] || []; const i = a.indexOf(val); i >= 0 ? a.splice(i, 1) : a.push(val); d.ycBuild[cat] = a; }
    else { d.ycBuild[cat] = d.ycBuild[cat] === val ? null : val; }
    await saveData(d); refreshXhs();
  }
  async function ycGen() {
    let d = loadData();
    const cust = (readInputCache('yc-custom') || '').trim();
    if (cust) { d.ycBuild = d.ycBuild || {}; d.ycBuild.custom = cust; await saveData(d); }
    const prompt = buildYcPrompt(loadData());
    toastr.info('正在生成形象…十几秒,别急~');
    const url = await genCharImage(prompt);
    if (!url) { toastr.error('没拿到图。先在酒馆聊天框试 /sd 1girl 看能不能出图'); return; }
    const d2 = loadData();
    const _gf = ((d2.ycBuild || {}).frame) || '';
    d2.charPortrait = { url, prompt, at: Date.now(), frame: _gf === 'full body' ? 'full' : (_gf === 'cowboy shot' ? 'cowboy' : (_gf === 'upper body' ? 'half' : 'full')) };
    d2.currentRoute = 'yc-home';
    await saveData(d2); refreshXhs();
    toastr.success('形象生成好啦!');
  }
  async function ycClearPortrait() {
    if (!TOP.confirm('确定清除 ta 当前的形象?')) return;
    const d = loadData(); d.charPortrait = null; await saveData(d); refreshXhs();
  }
  async function ycStartAnno() {
    const d = loadData();
    if (!d.charPortrait) { toastr.error('先有形象再标注~'); return; }
    d.ycAnno = { idx: 0, placed: [] };
    d.currentRoute = 'yc-annotate';
    await saveData(d); refreshXhs();
  }
  async function ycAnnoFinishData(d) {
    const placed = (d.ycAnno && d.ycAnno.placed) || [];
    if (d.charPortrait) d.charPortrait.hotspots = placed;
    d.ycAnno = null; d.currentRoute = 'yc-home';
    await saveData(d); refreshXhs();
    toastr.success(placed.length ? ('标好啦!已标 ' + placed.length + ' 处,戳得准了~') : '没标点,先用通用摸摸~');
  }
  async function ycAnnoTap(e, $btn) {
    const stage = $btn && $btn[0]; if (!stage) return;
    const nm = ycImgNorm(e, stage);
    const d = loadData(); if (!d.ycAnno) return;
    const part = YC_PARTS[d.ycAnno.idx];
    if (part) {
      d.ycAnno.placed = (d.ycAnno.placed || []).filter(p => p.key !== part.key);
      d.ycAnno.placed.push({ key: part.key, label: part.label, action: part.action, x: nm.nx, y: nm.ny });
      d.ycAnno.idx++;
    }
    if (d.ycAnno.idx >= YC_PARTS.length) { await ycAnnoFinishData(d); return; }
    await saveData(d); refreshXhs();
  }
  async function ycAnnoSkip() {
    const d = loadData(); if (!d.ycAnno) return;
    d.ycAnno.idx++;
    if (d.ycAnno.idx >= YC_PARTS.length) { await ycAnnoFinishData(d); return; }
    await saveData(d); refreshXhs();
  }
  async function ycAnnoBack() {
    const d = loadData(); if (!d.ycAnno) return;
    if (d.ycAnno.idx > 0) d.ycAnno.idx--;
    await saveData(d); refreshXhs();
  }
  async function ycAnnoFinish() { const d = loadData(); await ycAnnoFinishData(d); }
  function ycCutout(srcUrl) {
    return new Promise(function (resolve) {
      const TOPw = getTop();
      const im = new (TOPw.Image || Image)();
      im.crossOrigin = 'anonymous';
      im.onload = function () {
        try {
          let w = im.naturalWidth, h = im.naturalHeight;
          const max = 800; const sc = Math.min(1, max / Math.max(w, h));
          w = Math.max(1, Math.round(w * sc)); h = Math.max(1, Math.round(h * sc));
          const cv = TOPw.document.createElement('canvas'); cv.width = w; cv.height = h;
          const ctx = cv.getContext('2d'); ctx.drawImage(im, 0, 0, w, h);
          let img; try { img = ctx.getImageData(0, 0, w, h); } catch (e) { resolve(''); return; }
          const data = img.data, TH = 228;
          const isW = function (i) { return data[i] >= TH && data[i + 1] >= TH && data[i + 2] >= TH && Math.abs(data[i] - data[i + 1]) < 20 && Math.abs(data[i + 1] - data[i + 2]) < 20; };
          const visited = new Uint8Array(w * h);
          const stack = [];
          const seed = function (x, y) { if (x < 0 || y < 0 || x >= w || y >= h) return; const p = y * w + x; if (visited[p]) return; visited[p] = 1; if (isW(p * 4)) stack.push(p); };
          ...
          while (stack.length) {
            ...
            for (let k = 0; k < 4; k++) { const xx = nb[k][0], yy = nb[k][1]; if (xx < 0 || yy < 0 || xx >= w || yy >= h) continue; const q = yy * w + xx; if (visited[q]) continue; visited[q] = 1; if (isW(q * 4)) stack.push(q); }
          }
          ctx.putImageData(img, 0, 0);
          let out = ''; try { out = cv.toDataURL('image/png'); } catch (e) { out = ''; }
          resolve(out);
        } catch (e) { resolve(''); }
      };
  ...
  async function ycToggleCutout() {
    let d = loadData();
    if (!d.charPortrait || !d.charPortrait.url) { toastr.error('先有形象~'); return; }
    if (d.charPortrait.cutout) { d.charPortrait.cutout = false; await saveData(d); refreshXhs(); return; }
    if (!d.charPortrait.cutoutUrl) {
      toastr.info('正在抠白底…几秒钟~');
      const out = await ycCutout(d.charPortrait.url);
      if (!out) { toastr.error('这张抠不了(跨域链接图/复杂背景)。建议用本地导入或生成的白底图~'); return; }
      d = loadData(); if (!d.charPortrait) return;
      d.charPortrait.cutoutUrl = out;
    }
    d.charPortrait.cutout = true; await saveData(d); refreshXhs();
    toastr.success('抠好啦,ta 站进房间里了~');
  }
  async function ycVisionDescribe() {
    const ctx = getCtx();
    if (!ctx || !ctx.executeSlashCommandsWithOptions) { toastr.error('拿不到酒馆命令接口'); return; }
    if (!(loadData().charPortrait)) { toastr.error('先设个形象再让 AI 看~'); return; }
    toastr.info('待会选一下 ta 的图,AI 就看~');
    const p = '用中文简短描述这张图里的角色本人:发型发色、瞳色、表情、衣着配饰、正拿着或正在做的事、姿势构图。只写人物,40字内,逗号分隔,别写背景别客套。';
    let cap = '';
    try {
      const res = await ctx.executeSlashCommandsWithOptions('/caption quiet=true ' + p);
      cap = (res && (res.pipe != null ? res.pipe : (typeof res === 'string' ? res : ''))) || '';
    } catch (e) { toastr.error('看图失败:' + (e && e.message || e)); return; }
    cap = String(cap).replace(/<\/?[A-Za-z][^>]*>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 300);
    if (!cap) { toastr.error('没拿到描述。需先在 扩展→图像描述 把来源设成「多模态」并配好(可用免费的 Google AI Studio,或 Claude)'); return; }
    const d = loadData();
    if (d.charPortrait) { d.charPortrait.look = cap; await saveData(d); refreshXhs(); toastr.success('AI 看完啦!描述已存~'); }
  }
  async function ycImportUrl() {
    const u = (readInputCache('yc-imgurl') || '').trim();
    if (!u) { toastr.error('先填图片链接'); return; }
    const d = loadData();
    const _lk = (readInputCache('yc-look') || '').trim();
    d.charPortrait = { url: u, prompt: '(导入·链接)', look: _lk, at: Date.now() };
    d.currentRoute = 'yc-home';
    await saveData(d); try { clearInputCache('yc-imgurl'); } catch (_) {} refreshXhs();
    toastr.success('形象设置好啦!');
  }
  function ycHandleFile(file) {
    if (!file) return;
    if (!/^image\//.test(file.type || '')) { toastr.error('请选图片文件'); return; }
    const TOPw = getTop();
    const reader = new (TOPw.FileReader || FileReader)();
    reader.onload = function () {
      ...
      im.onload = async function () {
        try {
          ...
          w = Math.max(1, Math.round(w * sc)); h = Math.max(1, Math.round(h * sc));
          const cv = TOPw.document.createElement('canvas');
          cv.width = w; cv.height = h;
          cv.getContext('2d').drawImage(im, 0, 0, w, h);
          let url = '';
          try { url = cv.toDataURL('image/webp', 0.85); } catch (_) {}
          if (!url || url.indexOf('data:image/webp') !== 0) { try { url = cv.toDataURL('image/jpeg', 0.85); } catch (_) {} }
          if (!url) { toastr.error('图片处理失败'); return; }
          const d = loadData();
          const _lk = (readInputCache('yc-look') || '').trim();
          d.charPortrait = { url, prompt: '(导入·本地)', look: _lk, at: Date.now() };
          d.currentRoute = 'yc-home';
          await saveData(d); refreshXhs();
          toastr.success('形象导入好啦!');
        } catch (err) { toastr.error('处理出错:' + (err && err.message || err)); }
      };
      im.onerror = function () { toastr.error('这张图读不了'); };
      im.src = reader.result;
    };
    reader.onerror = function () { toastr.error('读文件失败'); };
    reader.readAsDataURL(file);
  }

  function renderYangchar(d) {
    const css = `<style>@keyframes ycBreathe{0%,100%{transform:translateY(0) scale(1)}50%{transform:translateY(-6px) scale(1.015)}}.yc-breathe{animation:ycBreathe 4.3s ease-in-out infinite}@keyframes ycHeart{0%{transform:translateY(0) scale(.5);opacity:0}18%{opacity:1;transform:translateY(-12px) scale(1.15)}100%{transform:translateY(-95px) scale(.85);opacity:0}}.yc-heart{position:absolute;pointer-events:none;z-index:30;will-change:transform,opacity;animation:ycHeart 1.35s ease-out forwards}@keyframes ycFloatTxt{0%{transform:translateY(0) scale(.9);opacity:0}13%{opacity:1;transform:translateY(-6px) scale(1)}78%{opacity:1}100%{transform:translateY(-72px) scale(1);opacity:0}}.yc-floattxt{position:absolute;max-width:70%;background:rgba(255,245,250,.96);border:1.5px solid #ffc6df;color:#a8497a;font-size:12.5px;line-height:1.5;padding:8px 12px;border-radius:14px;box-shadow:0 5px 16px rgba(200,120,170,.28);pointer-events:none;z-index:31;animation:ycFloatTxt 5s ease-out forwards}@keyframes ycPulse{0%,100%{transform:scale(.9);opacity:.55}50%{transform:scale(1.08);opacity:1}}.yc-thinking{animation:ycPulse 1s ease-in-out infinite !important;opacity:1 !important}</style>`;
    if (d.currentRoute === 'yc-annotate') return renderYcAnnotate(d);
    if (d.currentRoute === 'yc-build') return css + renderYcBuild(d);
    if (d.currentRoute === 'yc-source') return css + renderYcSource(d);
    if (d.currentRoute === 'yc-import') return css + renderYcImport(d);
    const cn = getCharName() || 'ta';
    const top = `<div style="display:flex;align-items:center;padding:12px 10px;background:linear-gradient(135deg,#ffd1e3,#c9b6f5);color:#5a3a55;flex-shrink:0"><div data-action="open-app" data-app="home" style="width:32px;font-size:24px;cursor:pointer">‹</div><div style="flex:1;text-align:center;font-size:17px;font-weight:700;letter-spacing:1px">🎀 养 ${esc(cn)}</div><div style="width:32px"></div></div>`;
    let body;
    if (d.charPortrait && d.charPortrait.url) {
      const aff = d.charAffection || { points: 0, day: '', count: 0, lastReply: '' };
      const lv = ycLevel(aff.points || 0);
      const left = Math.max(0, YC_DAILY_CAP - ((aff.day === ycToday()) ? (aff.count || 0) : 0));
      const bubble = aff.lastReply ? `<div style="background:#fff5fb;border:1.5px solid #ffd9ea;border-radius:14px;border-top-left-radius:4px;padding:11px 13px;margin-bottom:12px;font-size:13px;color:#7a5a72;line-height:1.65">💬 ${esc(ycCleanReply(aff.lastReply))}</div>` : '';
      const bar = `<div style="background:#fff;border-radius:14px;padding:12px 14px;margin-bottom:12px;box-shadow:0 3px 12px rgba(160,120,180,.08)"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:7px"><span id="yc-bar-lv" style="font-size:13.5px;font-weight:800;color:#b06a92">Lv.${lv.lv} ${esc(lv.name)}</span><span id="yc-bar-points" style="font-size:11.5px;color:#b3a5b8">好感 ${aff.points || 0}${lv.next != null ? ' / ' + lv.next : ''}</span></div><div style="height:9px;background:#f3e6f0;border-radius:5px;overflow:hidden"><div id="yc-bar-fill" style="height:100%;width:${lv.pct}%;background:linear-gradient(90deg,#ff9ec4,#b18cf0);border-radius:5px"></div></div><div id="yc-bar-left" style="font-size:11px;color:#b3a5b8;margin-top:7px">今天还能摸 ${left} 下${left === 0 ? ' (摸够啦,明天再来~)' : ''}</div></div>`;
      const _fr = (d.charPortrait.frame) || 'full';
      const _frChip = (v, lb) => `<button data-action="yc-frame" data-frame="${v}" style="border:1.5px solid ${_fr === v ? '#ff7aa2' : '#ece8f0'};background:${_fr === v ? '#fff0f5' : '#fafafa'};color:${_fr === v ? '#ff5a90' : '#999'};border-radius:14px;padding:5px 12px;font-size:11.5px;font-weight:${_fr === v ? 700 : 400};cursor:pointer">${lb}</button>`;
      const _annoN = (d.charPortrait.hotspots && d.charPortrait.hotspots.length) || 0;
      const annoRow = _annoN
        ? `<div style="display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:12px;flex-wrap:wrap"><span style="font-size:11.5px;color:#5a9a6a;font-weight:700">✅ 已标 ${_annoN} 处 · 戳得准</span><button data-action="yc-anno" style="border:none;background:#fff;border-radius:11px;padding:5px 12px;color:#7a9a8a;font-size:11px;cursor:pointer;box-shadow:0 2px 7px rgba(120,160,140,.12)">重新标注</button></div>`
        : `<div style="display:flex;align-items:center;justify-content:center;gap:9px;margin-bottom:12px;flex-wrap:wrap"><span style="font-size:11px;color:#b3a5b8">现在戳哪都是摸摸~</span><button data-action="yc-anno" style="border:none;background:linear-gradient(135deg,#5ec88a,#5aaed0);color:#fff;border-radius:11px;padding:6px 13px;font-size:11.5px;font-weight:700;cursor:pointer">✏️ 标注部位(戳得更准)</button></div>`;
      const _look = (d.charPortrait.look || (d.charPortrait.prompt && d.charPortrait.prompt.indexOf('(导入') !== 0 ? d.charPortrait.prompt : '') || '');
      const lookBtn = `<div style="display:flex;gap:8px;justify-content:center;margin-bottom:10px;flex-wrap:wrap"><button data-action="yc-look-edit" style="border:none;background:#fff;border-radius:12px;padding:7px 13px;color:#b06a92;font-size:11.5px;cursor:pointer;box-shadow:0 2px 8px rgba(160,120,180,.1)">📝 ${_look ? '改描述' : '写描述'}</button><button data-action="yc-vision" style="border:none;background:#fff;border-radius:12px;padding:7px 13px;color:#5a7a9a;font-size:11.5px;cursor:pointer;box-shadow:0 2px 8px rgba(120,150,200,.12)">🔍 让 AI 看图描述</button>${_look ? `<div style="flex-basis:100%;text-align:center;font-size:10.5px;color:#b3a5b8;margin-top:1px">现描述:${esc(_look.slice(0, 40))}${_look.length > 40 ? '…' : ''}</div>` : ''}</div>`;
      const _cut = !!(d.charPortrait.cutout && d.charPortrait.cutoutUrl);
      const _imgSrc = _cut ? d.charPortrait.cutoutUrl : d.charPortrait.url;
      const _innerBg = _cut ? 'transparent' : 'linear-gradient(165deg,#fff,#fbf2fb)';
      const _innerSh = _cut ? 'none' : '0 8px 24px rgba(160,120,180,.16)';
      const _imgFx = _cut ? 'filter:drop-shadow(0 5px 9px rgba(120,90,130,.3));' : '';
      const _fv = ycFullnessVal(d.charFeed);
      const fullBar = `<div style="background:#fff;border-radius:14px;padding:10px 14px;margin-bottom:12px;box-shadow:0 3px 12px rgba(160,120,180,.08)"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px"><span style="font-size:12.5px;font-weight:700;color:#c8943a">🍚 饱食度</span><span id="yc-full-txt" style="font-size:11.5px;color:#b3a5b8">饱食 ${_fv}</span></div><div style="height:9px;background:#f3ecd6;border-radius:5px;overflow:hidden"><div id="yc-full-fill" style="height:100%;width:${_fv}%;background:linear-gradient(90deg,#ffd27a,#ffb15a);border-radius:5px"></div></div>${_fv < 30 ? '<div style="font-size:11px;color:#e08a4a;margin-top:6px">ta 有点饿了,喂点东西吧~</div>' : ''}</div>`;
      const foodRow = `<div style="background:#fff;border-radius:14px;padding:11px 12px;margin-bottom:12px;box-shadow:0 3px 12px rgba(160,120,180,.08)"><div style="font-size:12px;color:#b06a92;font-weight:700;margin-bottom:8px">🍽️ 喂喂 ta</div><div style="display:flex;flex-wrap:wrap;gap:8px">` + YC_FOODS.map(f => `<button data-action="yc-feed" data-food="${f.key}" style="flex:1 0 21%;border:1.5px solid #f3e2d0;background:#fffdf8;border-radius:13px;padding:9px 2px;cursor:pointer;line-height:1.05"><div style="font-size:20px">${f.emoji}</div><div style="font-size:9.5px;color:#b08a5a;margin-top:1px;font-weight:600">${f.name}</div></button>`).join('') + `</div></div>`;
      body = `<div class="yc-stage" data-action="yc-tap" data-frame="${_fr}" style="position:relative;overflow:visible;margin-bottom:10px;cursor:pointer">
          <div style="border-radius:22px;overflow:hidden;background:${_innerBg};box-shadow:${_innerSh};aspect-ratio:3/4;display:flex;align-items:center;justify-content:center"><img class="yc-breathe" src="${esc(_imgSrc)}" style="width:100%;height:100%;object-fit:contain;pointer-events:none;${_imgFx}"/></div>
          <div style="text-align:center;font-size:11.5px;color:#c79ab5;margin-top:7px;pointer-events:none">👆 戳戳 / 摸摸 ta · 碰不同地方反应不一样</div>
        </div>
        ${annoRow}
        ${lookBtn}
        ${bubble}${bar}
        ...
        <div style="display:flex;gap:8px;margin-top:12px">
          <button data-action="yc-source" style="flex:1.5;border:none;border-radius:13px;padding:12px 4px;background:linear-gradient(135deg,#ff9ec4,#b18cf0);color:#fff;font-size:13px;font-weight:700;cursor:pointer">🔄 换形象</button>
          <button data-action="yc-cutout" style="flex:1.3;border:none;border-radius:13px;padding:12px 4px;background:${_cut ? 'linear-gradient(135deg,#9ad8b0,#7ac0d8)' : '#fff'};color:${_cut ? '#fff' : '#5a8a7a'};font-size:13px;font-weight:700;cursor:pointer;box-shadow:0 2px 8px rgba(120,160,140,.14)">${_cut ? '↩️ 还原底' : '✂️ 抠白底'}</button>
          <button data-action="yc-clear" style="flex:1;border:none;border-radius:13px;padding:12px 4px;background:#fff;color:#b06a92;font-size:13px;cursor:pointer;box-shadow:0 2px 8px rgba(160,120,180,.12)">清除</button>
        </div>`;
    } else {
      body = `<div style="background:#fbf2fb;border-radius:22px;aspect-ratio:3/4;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#b8a;margin-bottom:18px;text-align:center;padding:0 24px">
          <div style="font-size:40px;margin-bottom:10px">🎀</div>
          <div style="font-size:13.5px;color:#a98ab8;line-height:1.7">还没给 ${esc(cn)} 形象<br>选一种方式开始吧</div>
        </div>
        <button data-action="yc-build" style="width:100%;border:none;border-radius:14px;padding:15px;background:linear-gradient(135deg,#ff9ec4,#b18cf0);color:#fff;font-size:15px;font-weight:700;cursor:pointer;margin-bottom:11px">✨ NovelAI 生图捏形象</button>
        <button data-action="yc-import" style="width:100%;border:none;border-radius:14px;padding:15px;background:linear-gradient(135deg,#9ad8ff,#b6c5f5);color:#2f4f6a;font-size:15px;font-weight:700;cursor:pointer">🖼️ 导入自己的图片</button>`;
    }
    return css + top + `<div class="xhs-scroll" style="background:linear-gradient(180deg,#fbeef7 0%,#f6e7f3 58%,#efdcec 58%,#e8d2e7 100%);padding:16px">${body}</div>`;
  }

  function renderYcBuild(d) {
    const b = d.ycBuild || {};
    const top = `<div style="display:flex;align-items:center;padding:12px 10px;background:linear-gradient(135deg,#ff9ec4,#b18cf0);color:#fff;flex-shrink:0"><div data-action="yc-home" style="width:32px;font-size:24px;cursor:pointer">‹</div><div style="flex:1;text-align:center;font-size:16px;font-weight:700">捏形象</div><div style="width:32px"></div></div>`;
    const prompt = buildYcPrompt(d);
    const out = `<div style="position:sticky;top:0;z-index:5;background:linear-gradient(135deg,#fff,#fdf2f8);border:1.5px solid #f3d9e6;border-radius:14px;padding:10px 12px;margin-bottom:12px;box-shadow:0 4px 12px rgba(160,120,180,.12)"><div style="font-size:11px;color:#b59;font-weight:700;margin-bottom:5px">当前提示词</div><div style="font-size:11.5px;line-height:1.55;color:#7a5a72;word-break:break-word">${esc(prompt)}</div></div>`;
    const cats = YC_CATS.map(c => {
      const chips = c.opts.map(o => {
        const on = c.multi ? ((b[c.key] || []).indexOf(o[1]) >= 0) : (b[c.key] === o[1]);
        return `<button data-action="yc-tag" data-cat="${c.key}" data-val="${esc(o[1])}" data-multi="${c.multi ? 1 : 0}" style="border:1.5px solid ${on ? '#ff7aa2' : '#ece8f0'};background:${on ? '#fff0f5' : '#fafafa'};color:${on ? '#ff5a90' : '#333'};border-radius:16px;padding:7px 12px;font-size:12.5px;font-weight:${on ? 700 : 400};cursor:pointer">${esc(o[0])}</button>`;
      }).join('');
      return `<div style="background:#fff;border-radius:14px;box-shadow:0 3px 12px rgba(160,120,180,.07);padding:11px 13px;margin-bottom:10px"><div style="font-size:12.5px;font-weight:800;color:#8a6a92;margin-bottom:8px">${c.lab}</div><div style="display:flex;flex-wrap:wrap;gap:6px">${chips}</div></div>`;
    }).join('');
    const custom = `<input id="yc-custom" value="${esc((b.custom || ''))}" placeholder="✏️ 加自己的词(英文,逗号分隔,可留空)" style="width:100%;box-sizing:border-box;border:1.5px solid #ece8f0;border-radius:11px;padding:9px 11px;font-size:12.5px;outline:none;margin-bottom:12px"/>`;
    const genBtn = `<button data-action="yc-gen" style="width:100%;border:none;border-radius:14px;padding:15px;background:linear-gradient(135deg,#ff9ec4,#b18cf0);color:#fff;font-size:15px;font-weight:700;cursor:pointer">✨ 生成 ta(走 NovelAI)</button>`;
    const tip = `<div style="font-size:11px;color:#b3a5b8;text-align:center;margin-top:10px;line-height:1.6">出图走你酒馆配好的 NovelAI,约十几秒。<br>想换发色/眼睛 → 点另一个 → 重新生成即可。</div>`;
    return top + `<div class="xhs-scroll" style="background:#fdf6fc;padding:14px">${out}${cats}${custom}${genBtn}${tip}</div>`;
  }

  function renderYcSource(d) {
    const cn = getCharName() || 'ta';
    const top = `<div style="display:flex;align-items:center;padding:12px 10px;background:linear-gradient(135deg,#ffd1e3,#c9b6f5);color:#5a3a55;flex-shrink:0"><div data-action="yc-home" style="width:32px;font-size:24px;cursor:pointer">‹</div><div style="flex:1;text-align:center;font-size:16px;font-weight:700">给 ${esc(cn)} 换形象</div><div style="width:32px"></div></div>`;
    const card = (act, emoji, title, desc, grad) => `<div data-action="${act}" style="background:#fff;border-radius:18px;padding:18px 16px;margin-bottom:14px;box-shadow:0 5px 18px rgba(160,120,180,.12);cursor:pointer;display:flex;align-items:center;gap:14px"><div style="width:54px;height:54px;border-radius:15px;background:${grad};display:flex;align-items:center;justify-content:center;font-size:28px;flex-shrink:0">${emoji}</div><div style="flex:1"><div style="font-size:15px;font-weight:800;color:#7a4a6a;margin-bottom:3px">${title}</div><div style="font-size:11.5px;color:#a98ab8;line-height:1.5">${desc}</div></div><div style="font-size:20px;color:#d8b8d0">›</div></div>`;
    const body = card('yc-build', '✨', 'NovelAI 生图', '用提示词积木让 AI 生成 ta 的样子', 'linear-gradient(135deg,#ff9ec4,#b18cf0)')
               + card('yc-import', '🖼️', '导入自己的图片', '贴图片链接,或从相册/文件选一张', 'linear-gradient(135deg,#9ad8ff,#b6c5f5)');
    return top + `<div class="xhs-scroll" style="background:#fdf6fc;padding:16px">${body}<div style="font-size:11px;color:#b3a5b8;text-align:center;margin-top:6px;line-height:1.6">两种都能做出呼吸 + 点击爱心 + 摸摸回复~</div></div>`;
  }

  function renderYcImport(d) {
    const cur = (d.charPortrait && d.charPortrait.url && d.charPortrait.url.indexOf('data:') !== 0) ? d.charPortrait.url : '';
    const top = `<div style="display:flex;align-items:center;padding:12px 10px;background:linear-gradient(135deg,#9ad8ff,#b6c5f5);color:#33506a;flex-shrink:0"><div data-action="yc-source" style="width:32px;font-size:24px;cursor:pointer">‹</div><div style="flex:1;text-align:center;font-size:16px;font-weight:700">导入图片</div><div style="width:32px"></div></div>`;
    const urlBox = `<div style="background:#fff;border-radius:16px;padding:14px;margin-bottom:14px;box-shadow:0 4px 14px rgba(120,150,200,.1)"><div style="font-size:12.5px;font-weight:800;color:#5a7a9a;margin-bottom:8px">🔗 用图片链接</div><input id="yc-imgurl" value="${esc(cur)}" placeholder="https://.../ta.png" style="width:100%;box-sizing:border-box;border:1.5px solid #d8e4f0;border-radius:11px;padding:10px 12px;font-size:12.5px;outline:none;margin-bottom:10px"/><button data-action="yc-import-url" style="width:100%;border:none;border-radius:12px;padding:12px;background:linear-gradient(135deg,#7ab8f5,#9aa8f0);color:#fff;font-size:14px;font-weight:700;cursor:pointer">用这个链接</button></div>`;
    const fileBox = `<div style="background:#fff;border-radius:16px;padding:14px;margin-bottom:14px;box-shadow:0 4px 14px rgba(120,150,200,.1)"><div style="font-size:12.5px;font-weight:800;color:#5a7a9a;margin-bottom:8px">📁 从相册 / 文件选</div><label style="display:block;border:1.5px dashed #b8cae0;border-radius:12px;padding:18px;text-align:center;font-size:13px;color:#7a90b0;cursor:pointer">点这里选一张图<input id="yc-file" type="file" accept="image/*" style="display:none"/></label><div style="font-size:10.5px;color:#aab8c8;margin-top:8px;line-height:1.5">本地图会自动压到 ≤768px 再存,省空间。</div></div>`;
    const lookBox = `<div style="background:#fff;border-radius:16px;padding:14px;margin-bottom:14px;box-shadow:0 4px 14px rgba(120,150,200,.1)"><div style="font-size:12.5px;font-weight:800;color:#5a7a9a;margin-bottom:8px">📝 形象描述(可选,但强烈建议)</div><input id="yc-look" value="${esc((d.charPortrait && d.charPortrait.look) || '')}" placeholder="如:黑发、黑色背心、叼着烟、慵懒半躺在床上" style="width:100%;box-sizing:border-box;border:1.5px solid #d8e4f0;border-radius:11px;padding:10px 12px;font-size:12.5px;outline:none"/><div style="font-size:10.5px;color:#aab8c8;margin-top:7px;line-height:1.5">填一句 ta 在这张图里的样子,摸摸互动的反应就会贴合这张图。</div></div>`;
    const tip = `<div style="font-size:11px;color:#b3a5b8;text-align:center;line-height:1.6">导入后照样能呼吸、点击冒爱心、摸摸有回复~<br>竖图(3:4)效果最好。</div>`;
    return top + `<div class="xhs-scroll" style="background:#eef4fb;padding:16px">${urlBox}${fileBox}${lookBox}${tip}</div>`;
  }

  function renderYcAnnotate(d) {
    const a = d.ycAnno || { idx: 0, placed: [] };
    const total = YC_PARTS.length;
    const done = a.idx >= total;
    const part = YC_PARTS[a.idx];
    const url = (d.charPortrait && d.charPortrait.url) || '';
    const top = `<div style="display:flex;align-items:center;padding:12px 10px;background:linear-gradient(135deg,#a0d8b0,#9ac5e0);color:#2f5a4a;flex-shrink:0"><div data-action="yc-anno-done" style="width:32px;font-size:24px;cursor:pointer">‹</div><div style="flex:1;text-align:center;font-size:16px;font-weight:700">标注部位</div><div style="width:32px"></div></div>`;
    const dots = (a.placed || []).map(p => `<div style="position:absolute;left:${(p.x * 100).toFixed(1)}%;top:${(p.y * 100).toFixed(1)}%;transform:translate(-50%,-50%);pointer-events:none"><div style="width:15px;height:15px;border-radius:50%;background:#ff6aa0;border:2px solid #fff;box-shadow:0 1px 4px rgba(0,0,0,.35)"></div><div style="position:absolute;left:50%;top:17px;transform:translateX(-50%);white-space:nowrap;font-size:9px;color:#fff;background:rgba(0,0,0,.5);padding:1px 5px;border-radius:6px">${esc(p.label)}</div></div>`).join('');
    const stage = `<div class="yc-anno-stage" data-action="yc-anno-tap" style="position:relative;border-radius:18px;overflow:hidden;background:#000;aspect-ratio:3/4;cursor:crosshair;margin-bottom:14px"><img src="${esc(url)}" style="width:100%;height:100%;object-fit:contain;pointer-events:none"/>${dots}</div>`;
    let banner, ctrls;
    if (done) {
      banner = `<div style="background:#eafaf0;border:1.5px solid #b8e6c8;border-radius:13px;padding:14px;text-align:center;font-size:14px;color:#3a7a5a;font-weight:700;margin-bottom:12px">都标完啦 🎉 已标 ${(a.placed || []).length} 处</div>`;
      ctrls = `<button data-action="yc-anno-done" style="width:100%;border:none;border-radius:13px;padding:14px;background:linear-gradient(135deg,#5ec88a,#5aaed0);color:#fff;font-size:15px;font-weight:700;cursor:pointer">完成,去摸摸 ta</button>`;
    } else {
      banner = `<div style="background:#fff;border-radius:13px;padding:13px 14px;box-shadow:0 3px 12px rgba(120,160,140,.12);margin-bottom:12px;text-align:center"><div style="font-size:12px;color:#9ab0a4;margin-bottom:3px">第 ${a.idx + 1} / ${total} 个</div><div style="font-size:16px;font-weight:800;color:#3a7a5a">点一下 ta 的【${esc(part.label)}】</div></div>`;
      ctrls = `<div style="display:flex;gap:9px"><button data-action="yc-anno-back" style="flex:1;border:1.5px solid #d8e4dc;background:#fff;border-radius:12px;padding:12px;color:#7a9a8a;font-size:12.5px;cursor:pointer">↩ 上一个</button><button data-action="yc-anno-skip" style="flex:1;border:1.5px solid #d8e4dc;background:#fff;border-radius:12px;padding:12px;color:#7a9a8a;font-size:12.5px;cursor:pointer">没有·跳过</button><button data-action="yc-anno-done" style="flex:1.1;border:none;border-radius:12px;padding:12px;background:linear-gradient(135deg,#5ec88a,#5aaed0);color:#fff;font-size:12.5px;font-weight:700;cursor:pointer">完成</button></div>`;
    }
    const tip = `<div style="font-size:11px;color:#a8bcb2;text-align:center;margin-top:12px;line-height:1.6">在图上点 ta 对应部位的位置就行,点错可"上一个"重标。<br>这张没有的部位(半身图的腿/裆等)点"跳过"。随时可"完成"。</div>`;
    return top + `<div class="xhs-scroll" style="background:#f0f7f3;padding:16px">${banner}${stage}${ctrls}${tip}</div>`;
  }
    ...
    return `
      <div class="xhs-home" style="background:${grad};--home-fg:${esc(d.homeText || '#ffffff')}">
        ...
        <div class="xhs-home-grid">
          ...
          ${xichangOn() ? `<div class="xhs-app" data-action="open-yangchar"><div class="xhs-app-icwrap"><div class="xhs-app-ic" style="background:linear-gradient(135deg,#ffd1e3,#c9b6f5)">🎀</div></div><span class="xhs-app-name">养 ta</span></div>` : ''}
        </div>
      </div>`;
  }
    ...
    return `
      ...
      <div class="xhs-scroll" style="padding:14px">
        ...
        <details class="xhs-set-section">
          ...
          <button class="xhs-gen-btn" style="background:#fff3cd;color:#856404;margin-top:12px" data-action="test-persona">🔍 诊断:查手机读到了「我」的什么人设?(星座/生日老搞错就点这个)</button>
        </details>
...
严格 JSON: {"type":"text或image","title":"...","coverText":"...","content":"...","tags":["可选"]}`;
    ...
    if (!String(p.title || '').trim() && !String(p.content || '').trim() && !String(p.coverText || '').trim()) { toastr.warning(`${cname} 这条动态没生成出来(解析失败),再点一次试试`); return; }
    const fresh = loadData();
...
严格 JSON,不要解释: {"posts":[{"type":"...","title":"...","coverText":"...","content":"...","tags":[]${useST ? ',"storyTime":"早于现在的剧情时间"' : ''},"comments":[{"author":"陌生网友名","text":"评论","charReply":"(可选)${cname}对这条的回复,不回就别加"}]}]}`;
    ...
    arr.forEach((p, i) => {
      if (!String(p.title || '').trim() && !String(p.content || '').trim() && !String(p.coverText || '').trim()) return;
      const type = p.type === 'text' ? 'text' : 'image';
...
2. 别替「${d.userName}」(发消息的人本人)说话、别冒充 ta 发言;但 ta 上面刚说话时,群友要【接住并回应 ta、跟 ta 互动】,别全程自顾自聊把 ta 晾在一边。
...
2. 别替「${d.userName}」(楼主)说话、别冒充楼主发言;但楼主上面刚说了话时,群友要【接住并回应楼主、跟楼主互动】,别全程自顾自聊把楼主晾在一边
...
3. 严格 JSON,不要解释: {"texts":["短句1","短句2"]}`;
    ...
    const raw = await callXhsAPI(sys + storyTimeAsk(d), `对话记录(末尾是对面刚发来的,请【针对 ta 最后说的话正面回应、接住话头】,别答非所问、别自顾自换话题、别把对方晾着不理):\n${recent}`);
    if (!raw) return;
...
严格 JSON,不要解释: {"posts":[{"type":"...","title":"...","coverText":"...","content":"...","tags":[]${useST ? ',"storyTime":"早于现在的剧情时间"' : ''},"comments":[{"author":"陌生网友名","text":"评论","charReply":"(可选)${cname}对这条的回复,不回就别加"}]}]}`;
  ...
  function bindEvents() {
    ...
    // 输入缓存
    $doc.on('input.xhs', '#xhs-float input[id^="set-"], #xhs-float input[id^="xhs-"], #xhs-float input[id^="yc-"], #xhs-float textarea[id^="xhs-"]', function () {
      TOP.__xhsInputs = TOP.__xhsInputs || {};
      TOP.__xhsInputs[this.id] = this.value || '';
    });
    $doc.on('change.xhs', '#xhs-float #yc-file', function () { try { ycHandleFile(this.files && this.files[0]); } catch (e) {} });
    ...
    $doc.on('click.xhs', '#xhs-float [data-action]', async function (e) {
      ...
      try {
        ...
        if (action === 'yc-tap') { await ycTap(e, $btn); return; }
        if (action === 'yc-anno-tap') { await ycAnnoTap(e, $btn); return; }
        ...
        switch (action) {
          ...
          case 'open-yangchar': if (!xichangOn()) break; await openYangchar(); break;
          case 'yc-build': await ycToBuild(); break;
          case 'yc-home': await ycHome(); break;
          case 'yc-tag': await ycSetTag(cat, $btn.data('val'), $btn.data('multi')); break;
          case 'yc-gen': await ycGen(); break;
          case 'yc-clear': await ycClearPortrait(); break;
          case 'yc-feed': await ycFeed($btn.data('food')); break;
          case 'yc-cutout': await ycToggleCutout(); break;
          case 'yc-source': { const _d = loadData(); _d.currentRoute = 'yc-source'; await saveData(_d); refreshXhs(); break; }
          case 'yc-import': { const _d = loadData(); _d.currentRoute = 'yc-import'; await saveData(_d); refreshXhs(); break; }
          case 'yc-import-url': await ycImportUrl(); break;
          case 'yc-frame': { const _d = loadData(); if (_d.charPortrait) { _d.charPortrait.frame = $btn.data('frame'); await saveData(_d); refreshXhs(); } break; }
          case 'yc-vision': await ycVisionDescribe(); break;
          case 'yc-anno': await ycStartAnno(); break;
          case 'yc-anno-skip': await ycAnnoSkip(); break;
          case 'yc-anno-back': await ycAnnoBack(); break;
          case 'yc-anno-done': await ycAnnoFinish(); break;
          case 'yc-look-edit': { const _d = loadData(); if (_d.charPortrait) { const _cur = _d.charPortrait.look || ((_d.charPortrait.prompt && _d.charPortrait.prompt.indexOf('(导入') !== 0) ? _d.charPortrait.prompt : '') || ''; const _v = TOP.prompt('描述一下 ta 在这张图里的样子(发型/衣着/正在做的事…),AI 摸摸互动会参考:', _cur); if (_v != null) { _d.charPortrait.look = String(_v).trim(); await saveData(_d); refreshXhs(); toastr.success('形象描述已存,互动会更贴图~'); } } break; }
          case 'yc-touch': await ycTouch($btn.data('part')); break;
  ...
  if (typeof toastr !== 'undefined') {
    toastr.success('📱 芋圆机 v383 已加载,输入 /yuyuan 或点「芋圆机弹出」按钮打开', '', { timeOut: 3000 });
  }
